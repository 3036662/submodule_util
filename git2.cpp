...
bool Git2::Clone(const std::string& upstreamUrl, const std::string& path){
    ...
    int error= git_clone(&ptr_root_repo, upstreamUrl.c_str(), path.c_str(), NULL);
    if  (error){
...
int Git2::SubmouduleForeachCallbackC(git_submodule *sm,  const char *name_, void *ptrResultVector){
   ...
   ptrRes->emplace_back(nullptr,head_oid,std::move(head_oid_Str),std::move(name),std::move(path),std::move(url));
    return 0;
}
...
bool Git2::FetchRemote(const Submodule& sm){
    ...
    int error =git_remote_create(&tmpRemote,ptr_root_repo,remote_name.c_str(),sm.absolute_url.c_str());
    if (error){
...
bool Git2::CreateTag(const Submodule& sm, const std::string& tag_name){
    ...
    error = git_object_lookup (&ptr_tmp_obj,ptr_root_repo,&oid,GIT_OBJECT_ANY);
    if (error){
    ...
    error = git_tag_create_lightweight (&created_tag_oid, ptr_root_repo,tag_name.c_str(),ptr_tmp_obj,1);
...
bool Git2::MergeTag(const std::string& tag_name){
    ...
    int error = git_reference_lookup(&tag_reference,ptr_root_repo,
    ...
    error = git_annotated_commit_from_ref(&tag_commit, ptr_root_repo, tag_reference); //FREE
    if (error){
    ...
    git_merge(ptr_root_repo,(const git_annotated_commit **) &tag_commit, 1, &merge_opts, &checkout_opts);
    if (error){
    ...
    error=git_commit_lookup(&merge_commit,ptr_root_repo,annotated_oid); // TODO free git_commit_free
    git_annotated_commit_free(tag_commit);
    if (error){
        PrintLastError();
        return false;
